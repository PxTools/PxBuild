    ...
    if not model.showdecimals.has_value():
        if model.decimals.get_value() > 6:
            val_result.add_error(
                f"Value <{model.decimals.get_value()}> in decimals is not valid. When the keyword showdecimals is not specified, the value for decimals must be between 0 and 6."
            )
    return val_result
