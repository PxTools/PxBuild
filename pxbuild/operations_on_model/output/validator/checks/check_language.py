...
def check_language(model: PXFileModel) -> ValidationResult:
    ...
    if not (model.language.has_value() and model.languages.has_value()):
        ...
        val_result.add_error(
            f'Specified language code "{model.language._px_value._string}" in keyword language must be one of the codes in keyword languages: {model.languages._px_value}'
        )
    return val_result
