...
def test_check_lang_keys_none_fails():
    ...
    assert (
        'Specified language code "None" for keyword SUBJECT-AREA must be one of the codes in keyword languages: "no","en","fi"'
...
def test_check_lang_keys_unlisted_language_fails():
    ...
    assert not val_rep.is_valid
    assert (
        'Specified language code "sv" for keyword VALUES must be one of the codes in keyword languages: "no","en","fi"'
        in val_rep.error_msg
    )
