...
from pxtool.model.util._px_keytypes import _KeytypeVariableLang, _KeytypeContentLang, _KeytypeVariableValueLang
        ...
        for keyword_name in const.KEYWORDS_PYTHONIC_MAP:
            ...
            if keyword.is_present() and keyword.has_subkey:
                for key in keyword._value_by_key:
                    ...
                    elif type(key) == _KeytypeVariableLang: 
                        self.check_keytype_variable(key, keyword)
                    elif type(key) == _KeytypeVariableValueLang:
...
def check_subkeys(model:PXFileModel) -> ValidationResult:
