...
class HelperPxCodes:
    def __init__(self, inPxCodes: PxCodes, inLanguages: List[str]) -> None:
        ...
        for lang in inLanguages:
            self._sorted_valueitems[lang] = sort_valueitems_by_field(
                inPxCodes.valueitems, sortby, lang
            )
    ...
    def getLabels(self, language) -> List[str]:
        return [
            valueitem.label[language] for valueitem in self._sorted_valueitems[language]
        ]
